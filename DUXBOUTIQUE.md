...
6. **Prochaine Étape (Backend / SaaS) :** L'architecture actuelle (Context local) est un prototype avancé. La prochaine grande évolution est l'intégration de **Supabase** (PostgreSQL, Authentification, Row Level Security). Le but est le **Multi-tenancy** (gestion de plusieurs clients SaaS). Les appels locaux `dispatch()` devront progressivement être migrés vers des Server Actions / Appels API sécurisés par des Middlewares Next.js.
